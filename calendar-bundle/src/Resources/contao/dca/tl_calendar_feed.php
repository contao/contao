...
class tl_calendar_feed extends Backend
{
	...
	public function checkPermission()
	{
		...
		if (!System::isGranted(ContaoCalendarPermissions::USER_CAN_DELETE_FEEDS))
		{
		...
		switch (Input::get('act'))
		{
			...
			case 'delete':
			case 'show':
				if (!in_array(Input::get('id'), $root) || (Input::get('act') == 'delete' && !System::isGranted(ContaoCalendarPermissions::USER_CAN_DELETE_FEEDS)))
				{
			...
			case 'copyAll':
				...
				if (Input::get('act') == 'deleteAll' && !System::isGranted(ContaoCalendarPermissions::USER_CAN_DELETE_FEEDS))
				{
	...
	{
		return System::isGranted(ContaoCalendarPermissions::USER_CAN_CREATE_FEEDS) ? '<a href="' . $this->addToUrl($href . '&amp;id=' . $row['id']) . '" title="' . StringUtil::specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.svg/i', '_.svg', $icon)) . ' ';
	}
	...
	{
		return System::isGranted(ContaoCalendarPermissions::USER_CAN_DELETE_FEEDS) ? '<a href="' . $this->addToUrl($href . '&amp;id=' . $row['id']) . '" title="' . StringUtil::specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.svg/i', '_.svg', $icon)) . ' ';
	}
