...
final class DataContainerTools
{
    ...
    #[McpTool(name: 'contao_dc_describe_resource', description: 'Get the field JSON Schema and supported operations for a discovered resource. Operations describe API capabilities, not a guarantee of permission on a particular record. Only use supported operations and writable fields.', annotations: new ToolAnnotations(readOnlyHint: true, openWorldHint: false))]
    ...
    #[McpTool(name: 'contao_dc_list_records', description: 'List records of a discovered resource. Follow the pagination links in the result. Requires the list operation.', annotations: new ToolAnnotations(readOnlyHint: true, openWorldHint: false))]
    public function listRecords(string $resource, #[Schema(minimum: 1)] int $page = 1): CallToolResult
    {
        if ($page < 1) {
            throw new ToolCallException('The page must be at least 1.');
        }

        return $this->execute($this->getOperation($resource, 'list'), ['page' => $page]);
    }
    ...
    #[McpTool(name: 'contao_dc_create_record', description: 'Create a record. First describe the resource and supply writable fields from its schema, including required fields. Requires the create operation.', annotations: new ToolAnnotations(destructiveHint: false, openWorldHint: false))]
    ...
    #[McpTool(name: 'contao_dc_update_record', description: 'Update a record using JSON Merge Patch. First describe the resource and supply only the writable fields to change. Requires the update operation.', annotations: new ToolAnnotations(destructiveHint: true, openWorldHint: false))]
