        ...
        $onGetResponse = function (FragmentTemplate $template, Response|null $preBuiltResponse) use ($templateNameToView, $templateName, $isLegacyTemplate): Response {
            if ($isLegacyTemplate) {
                ...
                try {
                    ...
                    // Enhance exception if a modern template name is defined,
                    // but we still delegate to the legacy framework
                    if (null !== ($definedTemplateName = $this->options['template'] ?? null) && preg_match('/^Could not find template "\S+"$/', $e->getMessage())) {
                        throw new \LogicException(sprintf('Could neither find template "%s" nor the legacy fallback template "%s".', $definedTemplateName, $templateName), 0, $e);
                    }
