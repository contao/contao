	...
	autoFocusFirstInputField: function() {
		...
		var inputs = edit
			...
			.filter(function(item) {
				return !item.get('disabled') && item.isVisible() && item.get('type') !== 'checkbox' && item.get('type') !== 'radio' && item.get('type') !== 'submit' && item.get('type') !== 'image' && (! item.get('autocomplete') || item.get('autocomplete') === 'off');
			});
