...
class DC_Folder extends DataContainer implements ListableDataContainerInterface, EditableDataContainerInterface
{
	...
	public function __construct($strTable)
	{
		...
		{
			System::getContainer()->get('contao.monolog.system_logger')->error('Could not load data container configuration for "' . $strTable . '"');
  ...
  ' . $strButtons . '
	...
	public function cut($source=null)
	{
		...
		else
		{
			...
			System::getContainer()->get('contao.monolog.system_logger')->files('File or folder "' . $source . '" has been moved to "' . $destination . '"');
		}
	...
	public function copy($source=null, $destination=null)
	{
		...
		System::getContainer()->get('contao.monolog.system_logger')->files('File or folder "' . $source . '" has been copied to "' . $destination . '"');
	...
	public function delete($source=null)
	{
		...
		System::getContainer()->get('contao.monolog.system_logger')->files('File or folder "' . $source . '" has been deleted');
  ...
  ' . $strButtons . '
	...
	public function protect()
	{
		...
		if (file_exists($this->strRootDir . '/' . $this->intId . '/.public'))
		{
			...
			System::getContainer()->get('contao.monolog.system_logger')->files('Folder "' . $this->intId . '" has been protected');
		}
		else
		{
			$objFolder = new Folder($this->intId);
			...
			System::getContainer()->get('contao.monolog.system_logger')->files('The protection from folder "' . $this->intId . '" has been removed');
		}
	...
	protected function save($varValue)
	{
		...
		if ($this->strField == 'name')
		{
			...
			if (stripos($this->intId, '__new__') !== false)
			{
				...
				System::getContainer()->get('contao.monolog.system_logger')->files('Folder "' . $this->strPath . '/' . $varValue . $this->strExtension . '" has been created');
			}
			else
			{
				// Update the database
				...
				System::getContainer()->get('contao.monolog.system_logger')->files('File or folder "' . $this->strPath . '/' . $this->varValue . $this->strExtension . '" has been renamed to "' . $this->strPath . '/' . $varValue . $this->strExtension . '"');
			}
