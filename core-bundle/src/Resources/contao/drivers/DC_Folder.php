	...
	public function edit()
	{
		...
		if (!empty($boxes))
		{
			...
			foreach ($boxes as $k=>$v)
			{
				...
				{
					if (!isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$vv]) || (($GLOBALS['TL_DCA'][$this->strTable]['fields'][$vv]['exclude'] ?? null) && !$security->isGranted(ContaoCorePermissions::USER_CAN_EDIT_FIELD_OF_TABLE, $this->strTable . '::' . $vv)))
					{
						unset($boxes[$k][$kk]);
					}
				}
...
<div class="' . $class . '">';
				...
				foreach ($this->strPalette as $v)
				{
					...
					if (($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['exclude'] ?? null) && !$security->isGranted(ContaoCorePermissions::USER_CAN_EDIT_FIELD_OF_TABLE, $this->strTable . '::' . $this->strField))
					{
						continue;
					}
