...
class Input
{
	...
	public static function stripTags($varValue, $strAllowedTags='', $strAllowedAttributes='')
	{
		...
		{
			@trigger_error('Using Input::stripTags() with $strAllowedTags but without $strAllowedAttributes has been deprecated and will no longer work in Contao 5.0.', E_USER_DEPRECATED);
		...
		$varValue = preg_replace_callback(
			'@</?([^\s<>/]*)@',
			function ($matches) use ($strAllowedTags)
			{
	...
	private static function stripAttributes($strHtml, $strAllowedTags, $arrAllowedAttributes)
	{
		...
		if (isset($arrAllowedAttributes['*']) && \in_array('*', $arrAllowedAttributes['*'], true))
		{
			return $strHtml;
		}
		...
		return preg_replace_callback(
			'@</?([^\s<>/]*)([^<>]*)>?|-->|[>"\'=]+@',
			function ($matches) use ($strAllowedTags, $arrAllowedAttributes, &$blnCommentOpen, &$strOpenRawtext)
			{
				$strTagName = strtolower(isset($matches[1]) ? $matches[1] : '');
				...
				$arrAttributes = array_filter(
					$arrAttributes,
					function ($strAttribute) use ($strTagName, $arrAllowedAttributes)
					{
						// Skip if all attributes are allowed
						if (isset($arrAllowedAttributes[$strTagName]) && \in_array('*', $arrAllowedAttributes[$strTagName], true))
						{
							return true;
						}
						...
						{
							if (
								\in_array($strCandidate, isset($arrAllowedAttributes['*']) ? $arrAllowedAttributes['*'] : array(), true)
								|| \in_array($strCandidate, isset($arrAllowedAttributes[$strTagName]) ? $arrAllowedAttributes[$strTagName] : array(), true)
							) {
								return true;
							}
						}
	...
	private static function getAttributesFromTag($strAttributes)
	{
		...
		foreach ($matches as $arrMatch)
		{
			...
			$strValue = isset($arrMatch[2]) ? $arrMatch[2] : '';
