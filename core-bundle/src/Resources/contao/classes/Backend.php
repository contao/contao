...
<ul id="tl_breadcrumb">
  <li>' . implode(' › </li><li>', $arrLinks) . '</li>
	...
	public static function addPageIcon($row, $label, DataContainer $dc=null, $imageAttribute='', $blnReturnImage=false, $blnProtected=false)
	{
		...
		// Return the image
		return '<a href="contao/main.php?do=feRedirect&amp;page=' . $row['id'] . '" title="' . \StringUtil::specialchars($GLOBALS['TL_LANG']['MSC']['view']) . '"' . (($dc->table != 'tl_page') ? ' class="tl_gray"' : '') . ' target="_blank" rel="noreferrer noopener">' . \Image::getHtml($image, '', $imageAttribute) . '</a> ' . $label;
	}
