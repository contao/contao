        ...
        $treeBuilder
            ...
            ->children()
                ...
                ->scalarNode('component_dir')
                    ->info('The directory where the assets / contao components are installed.')
                    ->cannotBeEmpty()
                    ->defaultValue('assets')
                    ->validate()
                        ->ifTrue(static fn (string $v): int => preg_match('@^(app|bin|config|contao|plugins|public|share|system|templates|var|vendor|web)(/|$)@', $v))
                        ->thenInvalid('%s')
                    ->end()
                ->end()
