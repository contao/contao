...
class ShowAction implements ActionInterface
{
    public function render(DataContainer $dc): string
    {
        ...
        {
            $allowedFields = array_unique(array(...$allowedFields, ...array_keys($GLOBALS['TL_DCA'][$dc->table]['fields'])));
        }
        ...
        foreach ($fields as $i)
        {
            if (($GLOBALS['TL_DCA'][$dc->table]['fields'][$i]['inputType'] ?? null) == 'password' || ($GLOBALS['TL_DCA'][$dc->table]['fields'][$i]['eval']['doNotShow'] ?? null) || ($GLOBALS['TL_DCA'][$dc->table]['fields'][$i]['eval']['hideInput'] ?? null) || !\in_array($i, $allowedFields))
            {
                continue;
            }
            ...
            elseif ($i == 'tstamp' || ($GLOBALS['TL_DCA'][$dc->table]['fields'][$i]['eval']['rgxp'] ?? null) == 'datim' || \in_array($GLOBALS['TL_DCA'][$dc->table]['fields'][$i]['flag'] ?? null, array(DataContainer::SORT_DAY_ASC, DataContainer::SORT_DAY_DESC, DataContainer::SORT_DAY_BOTH, DataContainer::SORT_MONTH_ASC, DataContainer::SORT_MONTH_DESC, DataContainer::SORT_MONTH_BOTH, DataContainer::SORT_YEAR_ASC, DataContainer::SORT_YEAR_DESC, DataContainer::SORT_YEAR_BOTH)))
            {
