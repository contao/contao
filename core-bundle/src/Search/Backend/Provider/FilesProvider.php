...
class FilesProvider implements ProviderInterface
{
    ...
    public function updateIndex(IndexUpdateConfigInterface $trigger): iterable
    {
        ...
        $items = $this->filesStorage->listContents('', true);

        if (null !== ($lastIndexed = $trigger->getUpdateSince()?->getTimestamp())) {
            $items->filter(static fn (FilesystemItem $item): bool => ($item->getLastModified() ?? 0) > $lastIndexed);
        }
    ...
    public function convertDocumentToHit(Document $document): Hit|null
    {
        ...
        if (null !== ($item = $this->filesStorage->get($metadata['path']))) {
            return null;
        }
    ...
    {
        return $this->permissionCheckingFilesStorage->canAccess($hit->getDocument()->getMetadata()['path'] ?? '');
    }
}
