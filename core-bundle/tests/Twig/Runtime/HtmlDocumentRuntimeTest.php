...
class HtmlDocumentRuntimeTest extends TestCase
{
    ...
    public function testAddsTwigContentToTheResponseContext(): void
    {
        ...
        $runtime->add('<style>first</style>', DocumentLocation::stylesheets, 'theme');
        $runtime->add('<style>replacement</style>', DocumentLocation::stylesheets, 'theme');
        $runtime->add('<script src="/footer.js"></script>', DocumentLocation::endOfBody, 'footer');

        $tags = $head->all();

        $this->assertSame('<meta data-first>', $tags['contao.twig.head.0']);
    ...
    public function testFallsBackToLegacyGlobalsWithoutResponseContextBags(array $data): void
    {
        ...
        $runtime->add($data['content'], $data['location'], $data['identifier']);
