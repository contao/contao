...
class ImageSizeItemAccessVoterTest extends TestCase
{
    public function testVoter(): void
    {
        ...
        $voter = new ImageSizeItemAccessVoter($decisionManager);

        $this->assertTrue($voter->supportsAttribute(ContaoCorePermissions::DC_PREFIX.'tl_image_size_item'));
        ...
        $this->assertSame(
            ...
            $voter->vote(
                $token,
                new ReadAction('tl_image_size_item', ['user' => 2]),
                [ContaoCorePermissions::DC_PREFIX.'tl_image_size_item'],
            ),
        );
        ...
        $this->assertSame(
            ...
            $voter->vote(
                $token,
                new ReadAction('tl_image_size_item', ['user' => 3]),
                [ContaoCorePermissions::DC_PREFIX.'tl_image_size_item'],
            ),
        );
    }
