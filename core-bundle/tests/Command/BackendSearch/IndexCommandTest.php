...
class IndexCommandTest extends TestCase
{
    public function testTriggersUpdateWithoutParameters(): void
    {
        ...
        $backendSearch
            ->expects($this->once())
            ->method('reindex')
            ->with($this->callback(static fn (ReindexConfig $config): bool => !$config->getUpdateSince()))
        ;
    ...
    public function testTriggersUpdateWithUpdateSince(): void
    {
        ...
        $backendSearch
            ->expects($this->once())
            ->method('reindex')
            ->with($this->callback(static fn (ReindexConfig $config): bool => '01.01.2020' === $config->getUpdateSince()->format('d.m.Y')))
        ;
