  ...
  <p class="tl_help tl_tip' . $strClass . '">' . $return . '</p>';
	...
	protected function generateHeaderButtons($arrRow, $strPtable)
	{
		...
		foreach ($GLOBALS['TL_DCA'][$strPtable]['list']['operations'] as $k=> $v)
		{
			...
			{
				// Hide the toggle icon if the user does not have access to the field
				if ((($GLOBALS['TL_DCA'][$strPtable]['fields'][$params['field']]['toggle'] ?? false) !== true && ($GLOBALS['TL_DCA'][$strPtable]['fields'][$params['field']]['reverseToggle'] ?? false) !== true) || (DataContainer::isFieldExcluded($strPtable, $params['field']) && !System::getContainer()->get('security.helper')->isGranted(ContaoCorePermissions::USER_CAN_EDIT_FIELD_OF_TABLE, $strPtable . '::' . $params['field'])))
				{
					continue;
				}
