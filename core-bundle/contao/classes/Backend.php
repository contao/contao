...
<nav aria-label="' . $GLOBALS['TL_LANG']['MSC']['breadcrumbMenu'] . '">
  <ul id="tl_breadcrumb">
	...
	public static function addPageIcon($row, $label, DataContainer $dc=null, $imageAttribute='', $blnReturnImage=false, $blnProtected=false, $isVisibleRootTrailPage=false)
	{
		...
		$imageAttribute = trim($imageAttribute . ' data-icon="' . Controller::addStaticUrlTo(Image::getPath(Controller::getPageStatusIcon((object) array_merge($row, array('published'=>1))))) . '" data-icon-disabled="' . Controller::addStaticUrlTo(Image::getPath(Controller::getPageStatusIcon((object) array_merge($row, array('published'=>0))))) . '"');
