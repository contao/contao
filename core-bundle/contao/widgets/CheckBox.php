...
class CheckBox extends Widget
{
	...
	public function generate()
	{
		...
		foreach ($arrAllOptions as $i=>$arrOption)
		{
			...
			$arrOptions[] = '<div class="checkbox_toggler' . ($blnFirst ? '_first' : '') . '"><a href="' . Backend::addToUrl('cbc=' . $id) . '" class="' . $class . '" data-action="contao--scroll-offset#store" onclick="AjaxRequest.toggleCheckboxGroup(this,\'' . $id . '\');return false">' . Image::getHtml('chevron-right.svg', attributes: (new HtmlAttributes())->set('width', 16)->set('height', 16)) . '</a>' . $i . '</div><fieldset id="' . $id . '" class="tl_checkbox_container checkbox_options" style="display:' . $display . '" data-controller="contao--check-all"><input type="checkbox" id="check_all_' . $id . '" class="tl_checkbox" data-action="contao--check-all#toggleAll"> <label for="check_all_' . $id . '" class="check-all"><em>' . $GLOBALS['TL_LANG']['MSC']['selectAll'] . '</em></label>';
		...
		if ($this->multiple)
		{
			return \sprintf(
				...
				$this->xlabel,
				$this->strName,
				$blnCheckAll ? '<span><input type="checkbox" id="check_all_' . $this->strId . '" class="tl_checkbox" data-action="focus->contao--scroll-offset#store contao--check-all#toggleAll"' . ($this->onclick ? ' onclick="' . $this->onclick . '"' : '') . '> <label for="check_all_' . $this->strId . '" class="check-all"><em>' . $GLOBALS['TL_LANG']['MSC']['selectAll'] . '</em></label></span>' : '',
				implode(' ', $arrOptions),
				$this->wizard
			);
		}
	...
	{
		return \sprintf(
			'<span><input type="checkbox" name="%s" id="opt_%s" class="tl_checkbox" value="%s"%s%s data-action="focus->contao--scroll-offset#store%s"%s> <label for="opt_%s">%s%s%s</label>%s</span>',
			$this->strName . ($this->multiple ? '[]' : ''),
			$this->strId . '_' . $i,
			$this->multiple ? self::specialcharsValue($arrOption['value']) : 1,
			$this->isChecked($arrOption),
			$this->getAttributes(),
			$this->multiple ? ' contao--check-all#toggleInput' : '',
