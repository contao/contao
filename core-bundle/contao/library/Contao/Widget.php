...
abstract class Widget extends Controller
{
	...
	/**
	 * Describe the API representation of this widget's values
	 *
	 * @param array<string, mixed> $config The DCA field configuration
	 * @param array<string, mixed> $schema The schema inferred from the DCA
	 *
	...
	/**
	 * Convert a complete stored value to its API representation
	 */
	public static function convertToApiValue(mixed $value, array $config, array $schema): mixed
	{
		if (($schema['type'] ?? null) === 'array')
		{
			...
			if (\is_string($value))
			{
				$value = match (true)
				{
					($config['sql']['type'] ?? null) === 'json' => json_decode($value, true, 512, JSON_THROW_ON_ERROR),
					isset($config['eval']['csv']) => StringUtil::trimsplit($config['eval']['csv'], $value),
					default => StringUtil::deserialize($value, true),
				};
			}

			return array_map(static fn ($item) => static::convertToApiValue($item, $config, $schema['items'] ?? array()), $value);
		}
	...
	/**
	 * Prepare a complete API value for form submission, not direct storage
	 */
	public static function convertToApiFormValue(mixed $value, array $config, array $schema): mixed
	{
		...
		if (\is_array($value))
		{
			return array_map(static fn ($item) => static::convertToApiFormValue($item, $config, $schema['items'] ?? array()), $value);
		}

		return self::formatDateValue($value, $config) ?? (string) $value;
	}
