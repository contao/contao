...
<select name="tl_field" class="tl_select' . ($active ? ' active' : '') . '" data-controller="contao--slim-select">
' . implode("\n", $options_sorter) . '
</select>
<span>=</span>
...
<select name="tl_sort" id="tl_sort" class="tl_select" data-controller="contao--slim-select">
' . implode("\n", $options_sorter) . '
</select>
</div>';
	}
  ...
  <option value="all"' . Widget::optionSelected($this->limit, null) . '>' . $GLOBALS['TL_LANG']['MSC']['filterAll'] . '</option>';
			...
			$fields = '
<select name="tl_limit" class="tl_select' . ($active ? ' active' : '') . '" onchange="this.form.requestSubmit()" data-controller="contao--slim-select">
  ' . $options . '
</select> ';
		}
...
<strong>' . $GLOBALS['TL_LANG']['MSC']['showOnly'] . ':</strong> ' . $fields . '
	...
	protected function filterMenu($intFilterPanel)
	{
		...
		foreach ($sortingFields as $cnt=>$field)
		{
			...
			// Begin select menu
			$fields .= '
<select name="' . $field . '" id="' . $field . '" class="tl_select' . ($active ? ' active' : '') . '" data-placeholder="' . (\is_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['label'] ?? null) ? $GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['label'][0] : ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['label'] ?? null)) . '" data-controller="contao--slim-select">
