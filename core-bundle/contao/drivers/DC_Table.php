...
<select name="tl_field" class="tl_select' . ($active ? ' active' : '') . '" data-controller="contao--select">
' . implode("\n", $options_sorter) . '
</select>
<span>=</span>
...
<select name="tl_sort" id="tl_sort" class="tl_select" data-controller="contao--select">
' . implode("\n", $options_sorter) . '
</select>
</div>';
	}
  ...
  <option value="all"' . Widget::optionSelected($this->limit, null) . '>' . $GLOBALS['TL_LANG']['MSC']['filterAll'] . '</option>';
			...
			$fields = '
<select name="tl_limit" class="tl_select' . ($active ? ' active' : '') . '" onchange="this.form.requestSubmit()" data-controller="contao--select">
  ' . $options . '
</select> ';
		}
