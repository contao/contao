		...
		if (!empty($boxes))
		{
			...
			foreach ($boxes as $k=>$v)
			{
				...
				if (isset($legends[$k]))
				{
					...
					$legend = "\n" . '<legend data-action="click->contao--toggle-fieldset#toggle keydown->contao--toggle-fieldset#toggleWithKeyboard" tabindex="0">' . ($GLOBALS['TL_LANG'][$this->strTable][$key] ?? $key) . '</legend>';
				}
