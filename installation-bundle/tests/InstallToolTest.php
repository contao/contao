...
class InstallToolTest extends TestCase
{
    ...
    public function testRaisesErrorIfNonRunningInStrictMode(string $sqlMode, AbstractMySQLDriver $driver, string $expectedDriver): void
    {
        ...
        $installTool->hasConfigurationError($context);
    ...
    private function getInstallTool(string $sqlMode, AbstractMySQLDriver $driver = null): InstallTool
    {
        ...
        $versionRow = new \stdClass();
        $versionRow->Version = '8.0.0-system';

        $versionStatement = $this->createMock(Statement::class);
        $versionStatement
            ->method('fetch')
            ->with(\PDO::FETCH_OBJ)
            ->willReturn($versionRow)
        ;

        $connection
            ->method('query')
            ->with('SELECT @@version as Version')
            ->willReturn($versionStatement)
        ;
