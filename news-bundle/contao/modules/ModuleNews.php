...
abstract class ModuleNews extends Module
{
	...
	protected function parseArticle($objArticle, $blnAddArchive=false, $strClass='', $intCount=0)
	{
		...
		$objTemplate->text = Template::once(static fn () => '');
		...
		{
			$objTemplate->text = Template::once(static fn () => true);
			$objTemplate->hasText = Template::once(static fn () => null !== $url);
		...
		else
		{
			...
			$objTemplate->hasText = Template::once(static function () use ($objArticle, $url) {
				return null === $url ? false : ContentModel::countPublishedByPidAndTable($objArticle->id, 'tl_news') > 0;
			});
		}
