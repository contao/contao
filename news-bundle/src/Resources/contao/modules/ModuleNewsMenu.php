...
class ModuleNewsMenu extends ModuleNews
{
	...
	protected function compileYearlyMenu()
	{
		...
		// Get the dates
		$objDates = $this->Database->query("SELECT FROM_UNIXTIME(date, '%Y') AS year, COUNT(*) AS count FROM tl_news WHERE pid IN(" . implode(',', array_map('\intval', $this->news_archives)) . ")" . ((!BE_USER_LOGGED_IN || TL_MODE == 'BE') ? " AND published='1' AND (start='' OR start<='$time') AND (stop='' OR stop>'" . ($time + 60) . "')" : "") . " GROUP BY year ORDER BY year DESC");
	...
	protected function compileDailyMenu()
	{
		...
		// Get the dates
		$objDates = $this->Database->query("SELECT FROM_UNIXTIME(date, '%Y%m%d') AS day, COUNT(*) AS count FROM tl_news WHERE pid IN(" . implode(',', array_map('\intval', $this->news_archives)) . ")" . ((!BE_USER_LOGGED_IN || TL_MODE == 'BE') ? " AND published='1' AND (start='' OR start<='$time') AND (stop='' OR stop>'" . ($time + 60) . "')" : "") . " GROUP BY day ORDER BY day DESC");
